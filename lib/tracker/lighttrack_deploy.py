import copy
import numpy as np
from lib.utils.deploy_helper import get_subwindow_tracking, python2round, bgr2nv12_opencv


class Lighttrack_deploy(object):
    def __init__(self, config):
        super(Lighttrack_deploy, self).__init__()
    ...
    def init(self, im, target_pos, target_sz, model):
        ...
        # z_nv12 = bgr2nv12_opencv(z_crop)
        z_bgr = np.expand_dims(z_crop, axis=0)
        # z_bgr = np.expand_dims(np.transpose(z_crop, (2, 0, 1)), axis=0)
        # print('template shape: ', z_crop.shape)
        # net.template(z_nv12)
        net.template(z_bgr)

        window = np.outer(np.hanning(p.score_size), np.hanning(p.score_size))  # [16, 16]
    ...
    def track(self, state, im):
        ...
        # x_nv12 = bgr2nv12_opencv(x_crop)
        x_bgr = np.expand_dims(x_crop, axis=0)
        # x_bgr = np.expand_dims(np.transpose(x_crop, (2, 0, 1)), axis=0)
        # print('search shape: ', x_crop.shape)
        # target_pos, target_sz, _ = self.update(net, x_nv12, target_pos, target_sz * scale_z, window, scale_z, p)
    ...
    def update(self, net, x_crops, target_pos, target_sz, window, scale_z, p, debug=False):
        ...
        # print(bbox_pred[:, 0, 0])
        # print("x1 y1 x2 y2: {} {} {} {}".format(pred_x1.shape, pred_y1.shape, pred_x2.shape, pred_y2.shape))

        # print(pred_y2 - pred_y1)
        # print(self.sz_wh(target_sz))
