...
import lib.utils.augmentation as auger
import sys
sys.path.append('../')
...
class LightTrackMDataset(Dataset):
    def __init__(self, cfg, calibration=False):
        super(LightTrackMDataset, self).__init__()
        # pair information
    ...
    def grids_bias(self):
        ...
        x, y = np.meshgrid(
            np.arange(0, sz * self.stride, self.stride).astype(np.float32),
            np.arange(0, sz * self.stride, self.stride).astype(np.float32),
        )

        self.grid_to_search = {}
        self.grid_to_search_x = x + self.offset
        self.grid_to_search_y = y + self.offset
